...
{
    showroom.addCar(make_shared<Car>(100000, 110000, "Audi"));
    showroom.addCar(make_shared<Car>(110000, 120000, "Nissan"));
    showroom.addCar(make_shared<Car>(100000, 110000, "Chevrolet"));

    showroom.addWorker(make_shared<Salesman>("Vasya"));
    showroom.addWorker(make_shared<CommonWorker>("Petya", 1000));
...
void sellCar(CarShowroom& showroom)
{
    ...
    try
    {
        ...
        auto salesman = dynamic_pointer_cast<Salesman>(workerById);
...
{
    const int hoursInMonth = 24 * 30;
    auto nowTime = chrono::system_clock::now();
    auto monthAgoTime = nowTime - chrono::hours(hoursInMonth);
    auto soldCars = showroom.getSoldCarsListByPeriod(monthAgoTime, nowTime);
